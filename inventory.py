...
def open_inventory():
    ...
    def load_inventory():
        ...
        cur = conn.cursor()
        cur.execute("SELECT name, quantity, unit, alert_level FROM ingredients ORDER BY name ASC")
        rows = cur.fetchall()
        conn.close()

        for name, qty, unit, alert in rows:
            row_id = tree.insert("", tk.END, values=(name, qty, unit, alert))
            try:
    ...
    def add_ingredient():
        try:
            ...
            cur.execute(
                "INSERT INTO ingredients (name, quantity, unit, alert_level) VALUES (?, ?, ?, ?)",
                (name, qty, unit, alert)
            )
            conn.commit()
            conn.close()
            ...
            messagebox.showerror("Error", "Quantity and alert level must be numbers.", parent=win)
            win.lift(); win.focus_force()
    ...
    # ========== LOW STOCK WINDOW (PHASE 1 FEATURE) ==========

    def open_low_stock():
        low_win = tk.Toplevel(win)
        low_win.title("Low Stock Items")
        center_window(low_win, 600, 400)
        ...
        tree_ls["columns"] = ("Name", "Quantity", "Unit", "Alert Level")
        tree_ls.column("#0", width=0, stretch=tk.NO)
        tree_ls.column("Name", anchor=tk.W, width=220)
        tree_ls.column("Quantity", anchor=tk.CENTER, width=100)
        tree_ls.column("Unit", anchor=tk.CENTER, width=80)
        tree_ls.column("Alert Level", anchor=tk.CENTER, width=100)
        ...
        def load_low_items():
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT name, quantity, unit, alert_level "
                "FROM ingredients "
            ...
            for name, qty, unit, alert in rows:
                row_id = tree_ls.insert("", tk.END, values=(name, qty, unit, alert), tags=("low_stock",))
        ...
        def restock_from_low():
            ...
            def confirm_restock_ls():
                try:
                    ...
                    restock_win.destroy()
                    load_low_items()
                    load_inventory()  # refresh main inventory too
                    low_win.lift(); low_win.focus_force()
        ...
        btn_frame = tk.Frame(low_win, bg=low_win["bg"])
        btn_frame.pack(pady=10)

        tk.Button(btn_frame, text="Restock Selected", width=18, command=restock_from_low).grid(row=0, column=0, padx=5)
        tk.Button(btn_frame, text="Close", width=10, command=low_win.destroy).grid(row=0, column=1, padx=5)
